 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the Asri extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure Asri's `fetch()` calls are proxied, patch global fetch in debug mode.
